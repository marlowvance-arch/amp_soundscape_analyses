...
# Everglades-optimized 10-index pipeline
...
parse_filename <- function(fname) {
  ...
  tibble(
    ...
    date_yyyymmdd = tok[2],
    time_hhmmss   = tok[3],
    date_iso      = ymd_hms(paste(tok[2], tok[3]), tz = "UTC")
  )
}


# --------------------------
# 5. AUDIO LOADING (MONO + RESAMPLE)
# --------------------------
...
convert_amp_to_linear <- function(amp_raw) {
  # Convert raw spectrogram magnitude to dB, then to linear amplitude
...
# ---- BI (meanspec-based, 1–8 kHz, Hz domain) ----
compute_BI <- function(w) {
  ...
  freq <- as.numeric(ms[,1])   # Hz
  amp  <- as.numeric(ms[,2])
  
  bio_mask <- freq >= 1 & freq <= 8  # kHz
  
  if (!any(bio_mask)) return(NA_real_)
  
  band_db <- 20 * log10(amp[bio_mask] + 1e-12)
  BI_val  <- sum(band_db - min(band_db))
  
  BI_val
}

# ---- NDSI (meanspec-based; anthro 50–400 Hz, bio 1–8 kHz) ----
compute_NDSI <- function(w) {
  ...
  A_mask <- freq >= 0.05 & freq <= 0.4    # kHz
  B_mask <- freq >= 1    & freq <= 8      # kHz
...
valid_files <- filename_df %>% filter(!is.na(serial), !is.na(date_iso))
...
message("✅ Everglades acoustic index summary written to: ",
        cfg$io$index_summary_file)
